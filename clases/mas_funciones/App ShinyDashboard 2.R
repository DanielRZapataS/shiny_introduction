...
lista_libros <-
  read.csv("clases/word_cloud/lista_libros.csv", encoding = "UTF-8")
...
leer_texto <- function(libro){
  ...
  texto <- readLines(sprintf("./clases/word_cloud/libros/%s.txt", libro), 
                     encoding = "UTF-8")
...
ui <- dashboardPage(
  ...
  dashboardHeader(
    title = "Libros Proyecto Gutenberg",
    titleWidth = 300
  ),
  dashboardSidebar(
    ...
    sidebarMenu(id = "pestanas",
      
      menuItem("Estudio",tabName = "estudio", icon = icon("stats", lib = "glyphicon"), selected = T),
      menuItem("Modificación",tabName = "modificacion", icon = icon("pencil", lib = "glyphicon")),
      menuItem("Nuevo Libro",tabName = "nuevo", icon = icon("book", lib = "glyphicon"))
                             ...
                             tabsetPanel( type = "tabs",
                                          ...
                                          tabPanel("inicio",
                                                   ...
                                                   fluidRow(
                                                     ...
                                                     column(1, offset = 5,
                                                            actionButton("pag_inicial_up", "", icon = icon("arrow-up", lib = "glyphicon"))),
                                                     column(1,
                                                            actionButton("pag_inicial_down", "", icon = icon("arrow-down", lib = "glyphicon")))),
                                          ...
                                          tabPanel("final",
                                                   ...
                                                   fluidRow(
                                                     ...
                                                     column(1, offset = 5,
                                                            actionButton("pag_final_up", "", icon = icon("arrow-up", lib = "glyphicon"))),
                                                     column(1,
                                                            actionButton("pag_final_down", "", icon = icon("arrow-down", lib = "glyphicon")))),
               ...
               tabItem(tabName = "nuevo",
                           ...
                           fluidRow(
                             column(8,
                                    ...
                                    h5("Haz clic primero en 'Procesar' y cuando aparezca el check, en 'Guardar'")),
                             column(1,
                                    actionButton("procesar", "Procesar")),
                             column(1, offset = 1,
                                    actionButton("guardar", "Guardar", icon = icon("remove", lib = 'glyphicon')))
                           ),
                           fluidRow(
                             column(6,
                                    h4("Características"),
                             ...
                             column(6,
                                    fluidRow(
                                      column(6,h4("Muestra")),
                                      column(1,
                                             actionButton("pag_nuevo_up", "", icon = icon("arrow-up", lib = "glyphicon"))),
                                      column(1, offset = 1,
                                             actionButton("pag_nuevo_down", "", icon = icon("arrow-down", lib = "glyphicon")))),
                                    fluidRow(
                                      br(),
...
server <- function(input, output, session){
  ...
  output$primera <- renderValueBox({
    ...
    valueBox(
      value = ifelse(is.null(input$tabla_inicio_rows_selected), 
                     inicio(), 
  ...
  output$ultima <- renderValueBox({
    ...
    valueBox(
      value = ifelse(is.null(input$tabla_final_rows_selected),
                     final(),
  ...
  output$tabla_inicio <- renderDataTable({
    ...
    datatable(as.data.frame(texto_entero()),
              escape = FALSE,
  ...
  output$tabla_final <- renderDataTable({
    ...
    datatable(as.data.frame(texto_entero(), stringsAsFactor = F),
              escape = FALSE,
  ...
  observeEvent(
    input$modificar,
    {
      ...
      final_mod <- ifelse(is.null(input$tabla_final_rows_selected),
                          final(),
                          input$tabla_final_rows_selected)
      v$lista_libros <- read.csv("clases/word_cloud/lista_libros.csv", encoding = "UTF-8", stringsAsFactors = F)
      v$lista_libros[v$lista_libros$codigo == input$seleccion,]$inicio <- inicio_mod
      v$lista_libros[v$lista_libros$codigo == input$seleccion,]$final <- final_mod
      write.csv(v$lista_libros, "clases/word_cloud/lista_libros.csv", fileEncoding = "UTF-8", row.names = FALSE)
      showModal(modalDialog(
        title = "Líneas Modificadas",
        h6("Recuerda volver a presionar el botón 'Cambiar' para aplicar el resultado"),
        size = "m",
        easyClose = T,
        fade = T,
  ...
  output$UIcodigo <- renderUI({
    ...
    palabra_seleccionada <- titulo[which(nchar(titulo) == max(nchar(titulo)))]
    
    selectInput("codigo", "Escoge un código para el libro",
                choices = titulo, selected = palabra_seleccionada)
  ...
  observeEvent(
    input$guardar,
    {
      writeLines(texto_nuevo(), paste("./libros/",input$codigo, ".txt", sep = ""))
      ...
      write.csv(lista_libros, "clases/word_cloud/lista_libros.csv", row.names = F, fileEncoding = "UTF-8")
  ...
  output$titulo_nuevo_libro <- renderText({input$titulo})
  
  output$codigo_nuevo_libro <- renderText({input$codigo})
  ...
  output$texto_nuevo_libro <- renderText({
    ...
    if(lineas_nuevo() == 0) {
      ...
      texto_nuevo()[c(seq(inicio_nuevo() - v$paginacion_nuevo, 
