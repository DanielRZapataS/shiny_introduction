  ...
  observeEvent(
    input$modificar,
    {
      ...
      final_mod <- ifelse(is.null(input$tabla_final_rows_selected),
                          final(),
                          input$tabla_final_rows_selected)
      v$lista_libros <- read.csv("lista_libros.csv", encoding = "UTF-8", stringsAsFactors = F)
      ...
      write.csv(v$lista_libros, "lista_libros.csv", fileEncoding = "UTF-8", row.names = FALSE)
